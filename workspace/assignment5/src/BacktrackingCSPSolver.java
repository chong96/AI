...
public class BacktrackingCSPSolver implements CSPSolver {
	...
	protected <E> String selectUnassignedVariable(CSP<E> csp, Assignment<E> assignment) {
		...
		int randomNumber = rand.nextInt(csp.variables.size());
		while (assignment.containsKey(csp.variables.get(randomNumber))) {
			randomNumber = rand.nextInt(csp.variables.size());
		}
		return csp.variables.get(randomNumber);
	}
	...
	private <E> Assignment<E> backtrack(CSP<E> csp, Assignment<E> assignment) {
		...
		List<E> list = orderDomainValues(csp,var,assignment);
